...
export class SqlServerAdapter implements DatabaseAdapter {
    ...
    return this.withPool(connection, credentials.password, async (pool) => {
      ...
      const discovered = await Promise.allSettled(
        result.recordset.map(async ({ name }) => ({
          name,
          schemas: await this.getSchemasForDatabase(connection, credentials.password, name)
        }))
      );
  ...
  public async getColumns(connection: SavedConnection, credentials: ConnectionCredentials, table: { schema: string; table: string }): Promise<DatabaseColumn[]> {
    return this.withPool(connection, credentials.password, async (pool) => {
      ...
      return result.recordset.map(({ column_name, data_type, is_nullable }) => ({
        name: column_name,
        dataType: data_type,
  ...
  private async getSchemasForDatabase(connection: SavedConnection, password: string, database?: string): Promise<string[]> {
    return this.withPool(connection, password, async (pool) => {
  ...
  private async withPool<T>(
    ...
    const pool = new sql.ConnectionPool(this.createConfig(connection, password, databaseOverride));
    await pool.connect();
  ...
  private createConfig(connection: SavedConnection, password: string, databaseOverride?: string): SqlServerConfig {
    return {
