...
const LAST_SELECTED_CONNECTION_KEY =
  "sqlConnectionWorkbench.lastSelectedConnection";
...
export class ConnectionStore {
  ...
  public getConnections(): SavedConnection[] {
    const stored = this.context.globalState.get<SavedConnection[]>(
      CONNECTIONS_KEY,
      [],
    );
  ...
  public async saveConnection(
    ...
    const saved: SavedConnection = {
      ...
      visibleSchemas: [
        ...new Set(
          draft.visibleSchemas.map((schema) => schema.trim()).filter(Boolean),
        ),
      ],
      awsSecret:
    ...
    if (saved.authMode === "storedPassword") {
      ...
      if (!nextPassword && !hasExistingPassword) {
        throw new Error(
          "A password is required for a password-managed connection.",
        );
      }

      if (nextPassword) {
        await this.context.secrets.store(
          this.getPasswordKey(connectionId),
          nextPassword,
        );
      }
    } else {
  ...
  public async setLastSelectedConnectionId(
    connectionId?: string,
  ): Promise<void> {
    await this.context.globalState.update(
      LAST_SELECTED_CONNECTION_KEY,
      connectionId,
    );
  }

  public async toWebviewConnection(
    connection: SavedConnection,
  ): Promise<WebviewConnection> {
    return {
      ...connection,
      hasStoredPassword:
        connection.authMode === "storedPassword" &&
        (await this.hasPassword(connection.id)),
    };
  }
